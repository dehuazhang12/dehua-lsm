...
LSMEngine::LSMEngine(std::string path) : data_dir(path) {
  ...
  block_cache = std::make_shared<BlockCache>(
      Config::getInstance().getLsmBlockCacheCapacity(),
      Config::getInstance().getLsmBlockCacheK());
...
uint64_t LSMEngine::put(const std::string &key, const std::string &value,
  ...
  if (memtable.get_total_size() >=
      Config::getInstance().getLsmTolMemSizeLimit()) {
    return flush();
  }
  return 0;
}
...
uint64_t LSMEngine::flush() {
  ...
  if (level_sst_ids.find(0) != level_sst_ids.end() &&
      level_sst_ids[0].size() >=
          Config::getInstance().getLsmSstLevelRatio()) {
    full_compact(0);
  }
  ...
  // 3. 准备 SSTBuilder
  SSTBuilder builder(Config::getInstance().getLsmBlockSize(),
                     true); // 4KB block size
...
LSMEngine::full_l0_l1_compact(std::vector<size_t> &l0_ids,
  ...
  *l0_begin_ptr = l0_begin;
  ...
  return gen_sst_from_iter(l0_l1_begin,
                           Config::getInstance().getLsmPerMemSizeLimit() *
                               Config::getInstance().getLsmSstLevelRatio(),
                           1);
}
...
LSMEngine::full_common_compact(std::vector<size_t> &lx_ids,
  ...
  std::shared_ptr<ConcatIterator> old_lx_begin_ptr =
      std::make_shared<ConcatIterator>(lx_iters, 0);

  std::shared_ptr<ConcatIterator> old_ly_begin_ptr =
      std::make_shared<ConcatIterator>(ly_iters, 0);
...
LSMEngine::gen_sst_from_iter(BaseIterator &iter, size_t target_sst_size,
  ...
  auto new_sst_builder =
      SSTBuilder(Config::getInstance().getLsmBlockSize(), true);
  while (iter.is_valid() && !iter.is_end()) {
    ...
    if (new_sst_builder.estimated_size() >= target_sst_size) {
      ...
      new_sst_builder = SSTBuilder(Config::getInstance().getLsmBlockSize(),
                                   true); // 重置builder
    }
  }
...
size_t LSMEngine::get_sst_size(size_t level) {
  if (level == 0) {
    return Config::getInstance().getLsmPerMemSizeLimit();
  } else {
    return Config::getInstance().getLsmPerMemSizeLimit() *
           static_cast<size_t>(std::pow(
               Config::getInstance().getLsmSstLevelRatio(), level));
  }
}
