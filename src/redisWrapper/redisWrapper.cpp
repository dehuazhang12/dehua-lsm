...
get_fileds_from_hash_value(const std::optional<std::string> &field_list_opt) {
  ...
  if (!field_list.empty()) {
    // 去除前缀后才是字段列表
    std::string preffix = Config::getInstance().getRedisHashValuePreffix();
    field_list =
  ...
  while (std::getline(iss, token,
                      Config::getInstance().getRedisFieldSeparator())) {
...
std::string get_hash_value_from_fields(const std::vector<std::string> &fields) {
  ...
  oss << Config::getInstance().getRedisHashValuePreffix();
  for (size_t i = 0; i < fields.size(); ++i) {
    if (i > 0)
      oss << Config::getInstance().getRedisFieldSeparator();
    oss << fields[i];
  }
  return oss.str();
}
...
inline std::string get_hash_filed_key(const std::string &key,
  ...
  return Config::getInstance().getRedisFieldPrefix() + key + "_" + field;
}

inline bool is_value_hash(const std::string &key) {
  return key.find(Config::getInstance().getRedisHashValuePreffix()) == 0;
}

inline std::string get_explire_key(const std::string &key) {
  return Config::getInstance().getRedisExpireHeader() + key;
}

std::string get_zset_key_socre(const std::string &key,
                               const std::string &score) {
  std::ostringstream oss;
  oss << std::setw(Config::getInstance().getRedisSortedSetScoreLen())
      << std::setfill('0') << score;

  std::string formatted_score = oss.str();

  std::string res = Config::getInstance().getRedisSortedSetPrefix() + key +
...
inline std::string get_zset_key_elem(const std::string &key,
  ...
  return Config::getInstance().getRedisSortedSetPrefix() + key + "_ELEM_" +
         elem;
}

inline std::string get_zset_key_preffix(const std::string &key) {
  return Config::getInstance().getRedisSortedSetPrefix() + key + "_";
}

inline std::string get_zset_score_preffix(const std::string &key) {
  return Config::getInstance().getRedisSortedSetPrefix() + key + "_SCORE_";
}
...
inline std::string get_set_key_preffix(const std::string &key) {
  return Config::getInstance().getRedisSetPrefix() + key + "_";
}

inline std::string get_set_member_key(const std::string &key,
                                      const std::string &elem) {
  return Config::getInstance().getRedisSetPrefix() + key + "_" + elem;
}
...
std::string RedisWrapper::redis_lpush(const std::string &key,
  ...
  if (!list_value.empty()) {
    list_value =
        value + Config::getInstance().getRedisListSeparator() + list_value;
  } else {
    list_value = value;
  }
  ...
  return ":" +
         std::to_string(split(list_value,
                              Config::getInstance().getRedisListSeparator())
                            .size()) +
         "\r\n";
}
...
std::string RedisWrapper::redis_rpush(const std::string &key,
  ...
  if (!list_value.empty()) {
    list_value =
        list_value + Config::getInstance().getRedisListSeparator() + value;
  } else {
    list_value = value;
  }
...
std::string RedisWrapper::redis_lpop(const std::string &key) {
  ...
  std::vector<std::string> elements = split(
      list_opt.value(), Config::getInstance().getRedisListSeparator());
  if (elements.empty()) {
    return "$-1\r\n"; // 表示链表为空
  }
  ...
  if (elements.empty()) {
    ...
    lsm->put(key,
             join(elements, Config::getInstance().getRedisListSeparator()));
  }
